...
dataset = "Human_Skin_Automated_IBEX.ims"
output_dir = Path.home() / f"dryad_dcl_outputs/skin_ibex_automated"
...
data_chnames = np.array(
    [name.upper() for name in df["Target Name"].dropna() if not name.startswith("(")]
)
...
segmentation_input = np.stack(
    [
        ...
        img[data_ch_to_idx["CYTOKERATIN 10"]],
    ],
    axis=-1,
)[np.newaxis, ...]
