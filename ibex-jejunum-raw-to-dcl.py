...
dataset = "Human_Jejunum_Automated_IBEX.ims"
output_dir = Path.home() / f"dryad_dcl_outputs/jejunum_ibex_automated"
...
mpp = ims_data.resolution[-1]  # Pixel size, in microns (hopefully)  mpp = 0.16
...
segmentation_input = np.stack(
    [
        ...
        img[data_ch_to_idx["CYTOKERATIN (PAN)"]],
    ],
    axis=-1,
)[np.newaxis, ...]
...
# Tiling
for (xmin, xmax) in itertools.pairwise(np.linspace(0, X.shape[2], 6).astype(int)):
    for (ymin, ymax) in itertools.pairwise(np.linspace(0, X.shape[3], 7).astype(int)):
