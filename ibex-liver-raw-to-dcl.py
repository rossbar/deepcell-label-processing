...
dataset = "Human_Liver_Manual_IBEX.ims"
output_dir = Path.home() / f"dryad_dcl_outputs/liver_ibex_manual"
...
mpp = ims_data.resolution[-1]  # Pixel size, in microns (hopefully) mpp = 0.284
...
data_chnames = np.array(
    [name.upper() for name in df["Target Name"] if not name.startswith("See Above")]
)
...
# Tiling
for (xmin, xmax) in itertools.pairwise(np.linspace(0, X.shape[2], 8).astype(int)):
