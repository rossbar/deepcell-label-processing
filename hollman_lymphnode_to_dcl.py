...
df = pd.read_excel(fh, "Codex", skiprows=12, usecols=(1, 2), nrows=28)
# Extract to mapping
chmapping = dict(zip(df["Channel Name"], df["Marker"]))
# Filter channels
other_dapis = {f"DAPI{i}" for i in range(2, 8)}
chname_to_idx = {
...
for (xmin, xmax) in itertools.pairwise(np.linspace(0, X.shape[2], 4).astype(int)):
    for (ymin, ymax) in itertools.pairwise(np.linspace(0, X.shape[3], 4).astype(int)):
