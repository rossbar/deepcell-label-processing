...
data_dir = Path("/data2/omap-reference-datasets/omap-palatine-tonsil-10")
output_dir = Path.home() / f"dryad_dcl_outputs/omap10-tonsil/"
output_dir.mkdir(exist_ok=True, parents=True)

# Stack image from individual tiffs
img_dict = {
    fpath.name.split(".")[0]: tff.imread(fpath)
    for fpath in tqdm(data_dir.iterdir())
    if fpath.name.endswith(".tif")
}
...
segmentation_input = np.stack(
    [
        img[data_ch_to_idx["DAPI_01"]],
        img[data_ch_to_idx["HLADR_REAL550"]],
    ],
    axis=-1,
)[np.newaxis, ...]
...
# Segment
app = Mesmer()
mpp = 0.5  # TODO: Determine this value
...
# Tile
for (xmin, xmax) in itertools.pairwise(np.linspace(0, X.shape[2], 7).astype(int)):
